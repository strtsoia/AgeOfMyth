...
public class RecruitScreen extends Scene2D {
			...
			for (int col = 0; col < 4; col++) {
				...
				if (id <= boundary) {
					CoreImage[] img = new CoreImage[] {
							battleCardImg[row * 12 + col],
							battleCardImg[row * 12 + col + 4],
							battleCardImg[row * 12 + col + 8] };
					battleCardBtn[row][col] = new Button(img, col * width, row
							* height);
					...
					if (!EnoughResource(id) || !meetAgeRecruit(id)
			...
			for (int col = 0; col < 4; col++) {
				...
				if (ID <= boundary) {
					...
					if (!EnoughResource(ID) || !meetAgeRecruit(ID)
							|| maxNumOfRecruit == RecruitedNum
							|| !isAvailable(ID))
						battleCardBtn[row][col].setImage(battleCardImg[row * 12
								+ col + 8]);

					// recruit this unit
					if (battleCardBtn[row][col].isClicked()
							&& EnoughResource(ID) && meetAgeRecruit(ID)
							&& maxNumOfRecruit > RecruitedNum
							&& isAvailable(ID)) {
	...
	private boolean EnoughResource(int ID) {
		Hashtable<GlobalDef.Resources, Integer> cost = getUnitMap().get(ID)
				.getCost();
