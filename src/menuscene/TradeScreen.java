...
public class TradeScreen extends Scene2D {
	...
	public void load() {
		...
		if (payOver) {
			...
			for (int i = 0; i < 4; i++) {
				CoreImage[] pImg = new CoreImage[] { cubesImg[i], cubesImg[i],
						cubesImg[i] };
				...
				userSlider[i] = new Slider("slider.png", "slider-thumb.png",
						10, width * i);
			...
			for (int i = 0; i < 5; i++) {
				...
				bankSlider[i] = new Slider("slider.png", "slider-thumb.png",
						12, width * i);
	...
	public void update(int elapsedTime) {
		...
		} else // after pay cost
		{
			...
			// make sure user side resource should the same as amount of draw
			// from bank
			int userSideNum = 0;
			int bankSideNum = 0;
			for (int index = 0; index < 4; index++) {
			...
			if (okButton.isClicked() && userSideNum == bankSideNum) {
				...
				for (int index = 0; index < 4; index++) {
					int num = Integer.parseInt(userSideLabel[index].getText()
							.trim());
				...
				for (int index = 0; index < 4; index++) {
					int num = Integer.parseInt(bankSideLabel[index].getText()
							.trim());
