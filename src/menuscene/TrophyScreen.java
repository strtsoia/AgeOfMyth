...
public class TrophyScreen extends Scene2D{
	...
	public void load()
	{
		background = new ImageSprite("/battle/tropybackground.jpg", 200, 150, 400, 300); 
		add(background);
		...
		CoreImage[] resTileImg = CoreImage.load("/resource/resTile.jpg").split(12, 5);
		...
		// attack city area
		CoreImage[] buildTileImg = CoreImage.load("/resource/buildTile.jpg").split(12, 5);
	...
	public void update(int elapsedTime) 
	{
		...
		}else if(attackArea == 2)
		{
			...
			{
				if(this.holdButton.get(index).isMousePressed() && resSelection < 5 &&
						isResourceAvailableForPlayer(index))
				{
					resSelection++;
					int ID = holdButtonMapID.get(holdButton.get(index));
					int number = reTable.get(GlobalDef.getResourceMap().get(ID));
					number++;
					reTable.put(GlobalDef.getResourceMap().get(ID), number);
					ResourceHandler.Delete(defender.getGameBoard().getHoldResource(), reTable);
					ResourceHandler.Add(attacker.getGameBoard().getHoldResource(), reTable);
					clearReTable();
				}
			}
